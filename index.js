...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/loan/finalize') return json_(await finalizeLoan_(request, env), requestOrigin, env);
...
async function finalizeLoan_(request, env) {
  const data = await readJson_(request);
  ...
  const emailSent = await sendBorrowerEmail_(env, record, ticket.requestId, ticket.returnCode, items.results || []);
  return { ok: true, requestId: ticket.requestId, returnCode: ticket.returnCode, emailSent, message: emailSent ? '申請已送出，確認信已寄出。' : '申請已送出，請記下畫面的借用編號與歸還驗證碼。' };
}
...
async function lookupReturn_(request, env) {
  ...
  if (!isAdmin) {
    ...
    await requireReturnCode_(env.DB, requestId, data.returnCode);
  }
...
async function uploadPhoto_(request, env) {
  ...
  const session = await env.DB.prepare('SELECT consumed_at,expires_at FROM upload_sessions WHERE session_id=?').bind(ticket.sessionId).first();
  require_(session && !session.consumed_at && Number(session.expires_at) > Date.now(), 410, '照片上傳已逾時，請重新開始。');
...
async function requireReturnCode_(db, requestId, returnCode) {
  const row = await db.prepare('SELECT return_code_hash FROM requests WHERE request_id=?').bind(requestId).first();
...
async function rateLimit_(env, scope, maximum, windowMs) {
  ...
  require_(Number(result?.count || maximum + 1) <= maximum, 429, '嘗試次數過多，請稍後再試。');
}
...
function corsHeaders_(origin, env) { return { 'Access-Control-Allow-Origin': origin, 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type, X-Upload-Ticket, X-Photo-Stage, X-Equipment-Code, X-Photo-Index, X-Admin-Key', 'Access-Control-Max-Age': '600', Vary: 'Origin', 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'no-referrer' }; }
